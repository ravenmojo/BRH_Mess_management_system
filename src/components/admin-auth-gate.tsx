...
export function AdminAuthGate({ children, title = 'Admin Portal Access' }: AdminAuthGateProps) {
  ...
  // When unauthenticated: DO NOT load admin section at all. Render public dashboard blurred in background using Portal.
  if (!isAuthenticated) {
    if (!mounted) return null;

    return createPortal(
      <div className="fixed inset-0 z-[99999] overflow-hidden bg-slate-950 flex items-center justify-center p-4">
        ...
        {/* Dark Overlay Mask */}
        <div className="absolute inset-0 z-10 bg-slate-950/75 pointer-events-none" />

        {/* Centered Auth Popup Card */}
