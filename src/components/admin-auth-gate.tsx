...
function getStoredSession(): AdminSession | null {
  try {
    ...
    // Backward compatibility for boolean string
    if (raw === 'true') {
...
export function AdminAuthGate({ children, title = 'Admin Portal Access' }: AdminAuthGateProps) {
  ...
  const checkSession = () => {
    ...
    if (session && session.authenticated) {
      ...
      if (remaining > 0) {
        ...
        setError('Admin session expired (30 min limit). Re-enter password to continue working.');
        setTimeLeftMs(0);
        return false;
      }
    } else {
  ...
  return (
    <div className="relative space-y-4">
      ...
      {/* Children content (kept mounted so typed input state like remarks is never lost) */}
      <div className={!isAuthenticated ? 'pointer-events-none select-none blur-sm opacity-30 transition-all duration-300' : 'transition-all duration-300'}>
        {children}
      </div>
