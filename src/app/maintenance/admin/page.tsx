...
function MaintenanceAdminContent() {
  const { isAuthenticated, adminEmail, adminDesignation, isMasterAdmin, adminToken, canOverride, tier, canManageMaintenance } = useAdminAuth();
  const hasActionAccess = tier !== 'LOW' || Boolean(canManageMaintenance);
  ...
  const handleOverrideUserResolution = async (fb: any) => {
    ...
    const newOverallStatus = targetStatus && fb.adminResolved ? 'RESOLVED' : (targetStatus ? 'RESOLVED' : (fb.adminResolved ? 'RESOLVED' : 'PENDING'));
      ...
      <details className="group space-y-2" open>
        ...
        <div className="pt-2 space-y-3">
          ...
          <div className="w-full grid grid-cols-4 gap-1 p-1 bg-slate-100 dark:bg-slate-800/80 rounded-2xl border border-slate-200/80 dark:border-slate-700/60">
            ...
            >
              <div className="flex items-center space-x-1">
                <Layers className={`w-3.5 h-3.5 shrink-0 ${activeTab === 'ALL' ? 'text-sky-500' : 'text-slate-400'}`} />
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-1.5">
                      <div className="flex items-center space-x-1.5 flex-wrap gap-1">
                        ...
                        {fb.facilityType && (
                          <span className="px-2 py-0.5 rounded text-[10px] font-extrabold bg-sky-100 dark:bg-sky-950 text-sky-800 dark:text-sky-300 uppercase">
                            {fb.facilityType.replace('MAINTENANCE_', '')}
                          </span>
                        )}
                      </div>

                      {/* 2-Way Resolution Status Indicator */}
                      <div className="flex items-center space-x-1.5">
                        {isTwoWay ? (
                          <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-200 border border-emerald-300 dark:border-emerald-700 flex items-center space-x-1 shadow-xs">
                          ...
                          <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold bg-blue-100 dark:bg-blue-950 text-blue-800 dark:text-blue-300 border border-blue-200 dark:border-blue-800">
                            Admin Resolved (Pending Boarder)
                          </span>
                        ) : fb.userResolved ? (
                          <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold bg-indigo-100 dark:bg-indigo-950 text-indigo-800 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800">
                            Boarder Confirmed (Pending Admin)
                          </span>
                        ) : (
                          <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold bg-yellow-100 dark:bg-yellow-950 text-yellow-800 dark:text-yellow-300 border border-yellow-200 dark:border-yellow-800">
                            Unresolved (Pending)
                          </span>
                        )}
                      </div>
                      ...
                      <div className="space-y-1.5 pt-1 border-t border-slate-100 dark:border-slate-800/80">
                        ...
                        <div className="flex items-center space-x-1.5">
                          <input
                            ...
                            className="flex-1 px-3 py-1.5 rounded-xl text-xs bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-sky-500"
                          />
                          <button
                            type="button"
                        ...
                        <div className="flex items-center justify-between flex-wrap gap-2 pt-1">
                          <div className="flex items-center space-x-1.5 flex-wrap gap-1">
                            ...
                            {tier !== 'LOW' && (canOverride || isMasterAdmin) && (
                              <button
