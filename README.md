...
- **🖼️ Low-Res Preview Media & Download Options:** Added low-resolution image thumbnail and video player previews inline directly inside grievance cards across student portals and admin dashboards (`/feedback`, `/maintenance`, `/night-canteen`, `/maintenance/admin`, `/night-canteen/admin`), alongside inline **"Full Media"** links and **"Download"** buttons.
...
- **📈 Post-OTP Verification Uploading Progress Bar:** Immediately after OTP verification succeeds (or via cached 24h verification), submission forms display a live animated uploading progress bar (`Uploading Media Proof (XX%)...`) while uploading media to Cloudinary and saving to database.
